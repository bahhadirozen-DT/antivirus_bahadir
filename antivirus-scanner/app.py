...
def calculate_sha256(file_path):
    ...
    try:
        # Büyük dosyalarda takılmaması için okuma boyutunu büyüttük (64KB)
...
# --- ARAYÜZ (Geliştirilmiş ve Sabırlı JavaScript) ---
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="tr">
<head>
...
<body>
    ...
    <script>
        async function startScan() {
            ...
            statusText.innerText = "⚡ Klasör taranıyor, büyük klasörlerde bu işlem 5-10 saniye sürebilir...";
            resultsDiv.innerHTML = "";

            try {
                // Zaman aşımını engellemek için fetch kontrolü
                ...
                if (!data.results || data.results.length === 0) {
                    resultsDiv.innerHTML = "<p>Klasörde erişilebilir dosya bulunamadı.</p>";
                    return;
                }
                ...
                data.results.forEach((item, index) => {
                    ...
                    if (item.status === 'INFECTED') {
                        ...
                        p.innerHTML = `
                            ...
                            <div>
                                <button class="btn-quarantine" onclick="actionFile('quarantine', '${btoa(unescape(encodeURIComponent(item.file)))}', 'row-${index}')">Karantina</button>
                                <button class="btn-delete" onclick="actionFile('delete', '${btoa(unescape(encodeURIComponent(item.file)))}', 'row-${index}')">Sil</button>
                            </div>
                        `;
                    } else {
                        p.className = 'clean';
                ...
                statusText.innerText = "❌ Bağlantı zaman aşımına uğradı veya durduruldu."; 
                console.error(error);
            }
        }
...
def scan():
    ...
    # Dosya tarama döngüsünü daha esnek hale getirdik
...
def action():
    ...
    try:
        if action_type == 'delete':
            ...
            return jsonify({"success": True, "message": "Dosya karantinaya taşındı!"})
